...
N_MEASUREMENTS = 50
DATA_PATH = Path.home() / "Data/Derpy/09-18-2025/microscope_objective"
...
EXPERIMENT_NAME = "Microscope_Objective_NOT_CALIBRATION"
"""
---------------------------------
"""
...
if __name__ == "__main__":
    ...
    for experiment, N in zip(labels, nums):
        ...
        psg_power_measurements = []
        psa_power_measurements = []
        ...
        for i, angle in enumerate(tqdm(psg_angles, desc=msg)):
            ...
            if SAVE_PSG_IMGS:
                ...
                imstack, power_list = cam.take_many_images(N_MEDIANS, OPM=opm)
                ...
                psg_power_measurements.append(power_list)
            ...
            psa_power_measurements.append(power_list)

        # Save the images
        psg_cube = np.asarray(psg_images)
        psa_cube = np.asarray(psa_images)
        psg_power_measurements = np.asarray(psg_power_measurements)
        psa_power_measurements = np.asarray(psa_power_measurements)
        ...
        psg_power_hdu = fits.ImageHDU(data=psg_power_measurements, name="PSG_POWER_METER")
        psg_power_hdu.header['UNITS'] = "WATTS"
        
        psa_power_hdu = fits.ImageHDU(data=psa_power_measurements, name="PSA_POWER_METER")
        psa_power_hdu.header['UNITS'] = "WATTS"
        ...
        hdul = fits.HDUList([primary_hdu,
                             psg_hdu, psg_temp_hdu, psg_command_angles_hdu, psg_encoder_angles_hdu, psg_power_hdu,
                             psa_hdu, psa_temp_hdu, psa_command_angles_hdu, psa_encoder_angles_hdu, psa_power_hdu])
