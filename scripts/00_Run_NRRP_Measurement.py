...
if __name__ == "__main__":
    ...
    zaber_connection = ZaberConnection(ZABER_PORT_NRRP)
    psg = BaseZaberStage(zaber_connection, NRRP_PSG_ROTATION_STAGE_ID)
    psa = BaseZaberStage(zaber_connection, NRRP_PSA_ROTATION_STAGE_ID)
    ...
    for experiment, N in zip(labels, nums):
        ...
        psg_temp_hdu = fits.ImageHDU(
            data=np.asarray(psg_img_temps), name="PSG_TEMPERATURES"
        )
        psg_temp_hdu.header["UNITS"] = "CELSIUS"

        psg_command_angles_hdu = fits.ImageHDU(
            data=np.asarray(psg_command_angles), name="PSG_COMMAND_ANGLES"
        )
        psg_command_angles_hdu.header["UNITS"] = "DEGREES"

        psg_encoder_angles_hdu = fits.ImageHDU(
            data=np.asarray(psg_encoder_angles), name="PSG_ENCODER_ANGLES"
        )
        ...
        # I tried PrimaryHDU and it didn't let me name it. And I tried BinTableHDU and it
        # vomited out the numpy array and threw a big error.
        psa_temp_hdu = fits.ImageHDU(
            data=np.asarray(psa_img_temps), name="PSA_TEMPERATURES"
        )
        psa_temp_hdu.header["UNITS"] = "CELSIUS"

        psa_command_angles_hdu = fits.ImageHDU(
            data=np.asarray(psa_command_angles), name="PSA_COMMAND_ANGLES"
        )
        psa_command_angles_hdu.header["UNITS"] = "DEGREES"

        psa_encoder_angles_hdu = fits.ImageHDU(
            data=np.asarray(psa_encoder_angles), name="PSA_ENCODER_ANGLES"
        )
        ...
        hdul = fits.HDUList(
            [
                ...
                psg_hdu,
                psg_temp_hdu,
                psg_command_angles_hdu,
                psg_encoder_angles_hdu,
                psa_hdu,
                psa_temp_hdu,
                psa_command_angles_hdu,
                psa_encoder_angles_hdu,
                psa_power_hdu,
            ]
        )
        hdul.writeto(
            DATA_PATH / f"{experiment.lower()}_data_{todaysdate}_{todaystime}.fits",
            overwrite=False,
        )
