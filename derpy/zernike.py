...
def zernike_nm_seq(nms, r, t, norm=True):
    ...
    for nm, am_ in zip(nms, am):
        ...
        nj = (n-am_) // 2
        am_key = am_.item()
        if nj > jacobi_seqs_mjn[am_key]:
            jacobi_seqs_mjn[am_key] = nj

    for k in jacobi_seqs_mjn:
        nj = jacobi_seqs_mjn[k]
        jacobi_seqs_mjn[k] = np.arange(nj+1)
    ...
    for m in amu:
        m_key = m.item()
        powers_of_m[m_key] = r ** m
        sines[m_key] = np.sin(m*t)
        cosines[m_key] = np.cos(m*t)
...
def top_n(coefs, n=5):
    ...
    """
    coefsv = np.asarray(list(coefs.values()))
    ...
    names = np.asarray(names)[idxs]  # p = pair (n,m)
